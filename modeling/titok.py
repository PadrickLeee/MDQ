...
from modeling.quantizer.quantizer import VectorQuantizer, DiagonalGaussianDistribution, LookupFreeQuantizer
...
class TiTok(BaseModel, PyTorchModelHubMixin, tags=["arxiv:2406.07550", "image-tokenization"], repo_url="https://github.com/bytedance/1d-tokenizer", license="apache-2.0"):
    def __init__(self, config):
        ...
        self.quantize_mode = config.model.vq_model.get("quantize_mode", "lfq")
        if self.quantize_mode not in ["vq", "lfq","vae"]:
            raise ValueError(f"Unsupported quantize mode {self.quantize_mode}.")
        
        if self.finetune_decoder and self.quantize_mode not in ["vq", "lfq"]:
            raise ValueError("Only supprot finetune_decoder with vq/lfq quantization for now.")
    ...
    def encode(self, x):
        if self.finetune_decoder:
            with torch.no_grad():
                ...
                # result_dict["codebook_loss"] *= 0    # original
                result_dict["entropy_loss"] *= 0       # for LFQ
        else:
            z = self.encoder(pixel_values=x, latent_tokens=self.latent_tokens)
            if self.quantize_mode == "vq":
                z_quantized, result_dict = self.quantize(z)
