...
def create_model_and_loss_module(config, logger, accelerator,
    ...
    if config.experiment.get("init_weight", ""):
        ...
        if config.model.vq_model.finetune_decoder:
            ...
            ## new version
            ## add the TiTok VQGAN's quantizer/decoder weight as well
            pretrained_titok_weight = torch.load(
                "/home/ma-user/modelarts/work/jjw/SFT/lqf/pretrained_ckpt/TiTok/tokenizer_titok_l32.bin", map_location="cpu"
            )
            # Only keep the quantize and decoder part
            pretrained_titok_weight = {k:v for k,v in pretrained_titok_weight.items() if "pixel" in k}
            
            model_weight.update(pretrained_titok_weight)
            
            msg = model.load_state_dict(model_weight, strict=False)
        else:
            # add for finetune multi depth lfq tokenizer
            if config.model.vq_model.finetune_multi_depth:
                # Only keep the encoder and decoder part
                new_model_weight = {}
                for k, v in model_weight.items():
                    if 'quantize.' in k or 'pixel_' in k:
                        # create same shape zero tensor as original ckpt
                        new_model_weight[k] = torch.zeros_like(v)
                    else:
                        new_model_weight[k] = v  # keep original ckpt
                        
                msg = model.load_state_dict(new_model_weight, strict=False)
            
            # msg = model.load_state_dict(torch.load("/home/ma-user/modelarts/work/jjw/SFT/lqf/1d-tokenizer/titok_l32_stage1_run1/checkpoint-20000/unwrapped_model/pytorch_model.bin", map_location="cpu"), strict=False)
...
def create_evaluator(config, logger, accelerator):
    ...
    elif config.model.vq_model.get("quantize_mode", "vq") == "lfq":
        evaluator = VQGANEvaluator(
            device=accelerator.device,
            enable_rfid=True,
            enable_inception_score=True,
